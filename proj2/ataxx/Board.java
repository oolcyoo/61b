...
class Board {
    ...
    Board(Board board0) {
        ...
        _numJumpHistory = board0._numJumpHistory;
    ...
    void clear() {
        _whoseMove = RED;
        incrPieces(BLUE, -numPieces(BLUE));
        incrPieces(RED, -numPieces(RED));
        incrPieces(BLOCKED, -numPieces(BLOCKED));
        incrPieces(EMPTY, -numPieces(EMPTY));
        _allMoves = new ArrayList<>();
        for (char i = 'a'; i <= 'g'; i++) {
            for (char j = '1'; j <= '7'; j++) {
                boolean blue = ((i == 'a' & j == '1') | (i == 'g' & j == '7'));
                boolean red = (i == 'a' & j == '7' | (i == 'g' & j == '1'));
                if (blue) {
                    unrecordedSet(i, j, BLUE);
                    incrPieces(BLUE, 1);
                } else if (red) {
                    unrecordedSet(i, j, RED);
                    incrPieces(RED, 1);
                } else if (!(blue | red)) {
                    set(i, j, EMPTY);
                    incrPieces(EMPTY, 1);
    ...
    PieceColor getWinner() {
        boolean B = numPieces(BLUE) == 0;
        boolean R = numPieces(RED) == 0;
        if (numJumps() >= JUMP_LIMIT
                || (!canMove(RED) && !canMove(BLUE) || B || R)) {
    ...
    boolean legalMove(Move move) {
        ...
        if (whoseMove() != get(move.col0(), move.row0())) {
            return false;
        } else if (move.col1() < 'a' || move.row1() < '1') {
            return false;
        } else if (move.col1() > 'g' || move.row1() > '7') {
            return false;
    ...
    void makeMove(Move move) {
        ...
        if (move.isJump()) {
            ...
            _numJumps += 1;
            _numJumpHistory.push(_numJumps);
        }
        if (move.isExtend()) {
            set(move.col1(), move.row1(), _whoseMove);
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                char newC = (char) (move.col1() + i);
                char newR = (char) (move.row1() + j);
                if (get(newC, newR) == opponent) {
                    set(newC, newR, _whoseMove);
                    incrPieces(_whoseMove, 1);
    ...
    /** Undo the last move. */
    void undo() {
        Integer sq = _undoSquares.pop();
        PieceColor piece = _undoPieces.pop();
        Integer numJ = _numJumpHistory.pop();
        while (sq != null) {
            incrPieces(get(sq), -1);
            unrecordedSet(sq, piece);
            incrPieces(piece, 1);
            _numJumps = numJ;
            sq = _undoSquares.pop();
            piece = _undoPieces.pop();
            numJ = _numJumpHistory.pop();
        }
        _whoseMove = _whoseMove.opposite();
    ...
    int totalOpen() {
        int real = numPieces(EMPTY) + numPieces(RED)
                + numPieces(BLUE) - numPieces(BLOCKED);
        return real;
    }
     ...
     *  game). */
    List<Move> allMoves() {
        _allMoves = new ArrayList<>(_allMoves);
        return _allMoves;
    }
    ...
    public boolean equals(Object obj) {
        ...
        boolean X = Arrays.equals(_board, other._board);
        boolean Y = _whoseMove.equals(other._whoseMove);
        boolean Z = _allMoves.equals(other._allMoves);
        return X && Y && Z;
    }
    ...
    /** Stack of pieces formally at corresponding squares in.
     *  _UNDOSQUARES. */
    private Stack<PieceColor> _undoPieces;
    /** Stack of number of jumps history. **/
    private Stack<Integer> _numJumpHistory;
}
