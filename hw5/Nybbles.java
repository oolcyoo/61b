...
public class Nybbles {
    ...
    public Nybbles(int N) {
        _data = new int[(N + NYBBLES_PER_INT - 1) / NYBBLES_PER_INT];
        _n = N;
    }
    ...
    public int get(int k) {
        if (k < 0 || k >= _n) {
            ...
            int word = k / NYBBLES_PER_INT;
            int offset = (k % NYBBLES_PER_INT) * BITS_PER_NYBBLE;
            int value = (_data[word] >>> offset) & NYBBLE_MASK;
            if (value > MAX_VALUE) {
                return value - (1 << BITS_PER_NYBBLE);
            }
            return value;
        }
    }
    ...
    public void set(int k, int val) {
        if (k < 0 || k >= _n) {
            ...
            int word = k / NYBBLES_PER_INT;
            int offset = (k % NYBBLES_PER_INT) * BITS_PER_NYBBLE;
            int mask = NYBBLE_MASK << offset;
            _data[word] = (_data[word] & ~mask)
                    | ((val & NYBBLE_MASK) << offset);
        }
    }
